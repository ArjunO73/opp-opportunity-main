({
    modifyCalendarDOM : function(){
        $(".fc-row.fc-week.fc-widget-content.fc-rigid .fc-content-skeleton").each(function( index1 ) {
            $(this).children().find("tbody tr:first td").each(function( index ) {
                $(this).addClass('hide-tbody');
                index = index+1;
                if($(this).hasClass("fc-event-container"))
                console.log( index + ": " + $(this).parents('.fc-content-skeleton').find('table > thead > tr > td:nth-child('+index+')').addClass('cal-event-circle') );
            });
        }); 
    },
